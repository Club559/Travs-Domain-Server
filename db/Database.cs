...
SELECT MAX(chestId) FROM vaults WHERE accId = @accId;UPDATE accounts SET vaultCount=vaultCount+1 WHERE id=@accId;";
        ...
        public byte[] ReadHome(Account acc)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText = "SELECT data FROM homes WHERE accId=@accId;";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
            using (MySqlDataReader rdr = cmd.ExecuteReader())
            {
                if (rdr.HasRows)
                {
                    rdr.Read();
                    return ZlibStream.UncompressBuffer(
        ...
        public void CreateHome(Account acc, byte[] dat)
        {
            if (acc.Home == null)
                return;
            MySqlCommand cmd = CreateQuery();

            cmd.CommandText = @"INSERT INTO homes(accId, data) VALUES(@accId, @data)";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);

            var data = Convert.ToBase64String(
                ZlibStream.CompressBuffer(acc.Home));
            cmd.Parameters.AddWithValue("@data", data);

            cmd.ExecuteNonQuery();
        }

        public void SaveHome(Account acc, byte[] dat)
        {
            if (acc.Home == null)
                return;
            MySqlCommand cmd = CreateQuery();

            cmd.CommandText = @"UPDATE homes SET data=@data WHERE accId=@accId";
            cmd.Parameters.AddWithValue("@accId", acc.AccountId);
...
WHERE accId=@accId AND charId=@charId;";
            ...
            cmd.Parameters.AddWithValue("@nexus", chr.Nexus);
            chr.PCStats =
