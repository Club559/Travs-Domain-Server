    ...
    public class DamageCounter
    {
        ...
        public void Death(RealmTime time)
        {
            ...
            foreach (var i in eligiblePlayers)
            {
                ...
                //float playerXp = (((i.Item1.Level*4) + (enemy.ObjectDesc.MaxHP/10f)) / 2)*(enemy.ObjectDesc.ExpMultiplier ?? 1);

                float playerXp = (enemy.ObjectDesc.MaxHP / 10f) * (enemy.ObjectDesc.ExpMultiplier ?? 1);
                ...
                if (i.Item1.EnemyKilled(enemy, (int)playerXp, killer, LastProjectile != null ? LastProjectile.SlotId : -1) && !killer)
                    lvUps++;
                i.Item1.QuestCheckDeath(enemy.ObjectDesc.ObjectId);
            }
            if(LastHitter != null)
