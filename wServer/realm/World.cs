...
namespace wServer.realm
{
    public abstract class World: IDisposable
    {
        ...
        public virtual void HandleMapEdit(Player player, int mode, ushort type, int x, int y)
        {
            if (mode == 0)
                EditTile(player, type, x, y);
            if (mode == 1)
                EditObject(player, type, x, y);
        }

        public virtual void EditTile(Player player, ushort type, int x, int y)
        {
        ...
        public virtual void EditObject(Player player, ushort type, int x, int y)
        {
            var tile = Map[x, y].Clone();
